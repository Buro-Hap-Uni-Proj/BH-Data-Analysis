...
BHData$date<- as.IDate(BHData$datetime)
BHData$time_of_day<-as.ITime(BHData$datetime)

```

Filtering for one day:
```{r}
TESTDATA<-subset(BHData,date=="2019-12-18")
```
...
ggplot(data=realjourneys,aes(x=realjourneys$journeytime))+geom_histogram(binwidth = 10)+scale_y_continuous(limits = c(1,50))
...
summary(timeinfo$n)
#ggplot(data=timeinfo,aes(x=Ntod,y=n))+geom_point()
```
