...
TESTDATA$location[(TESTDATA$X<550 & TESTDATA$X>450 & TESTDATA$Y>0 & TESTDATA$Y<495) | (TESTDATA$X<370 & TESTDATA$X>310 & TESTDATA$Y>0 & TESTDATA$Y<70) | (TESTDATA$X<450 & TESTDATA$X>370 & TESTDATA$Y>10 & TESTDATA$Y<115)]<-"Desks"
TESTDATA$location[TESTDATA$X<200 & TESTDATA$X>0 & TESTDATA$Y>150 & TESTDATA$Y<270]<-"Kitchen"
TESTDATA$location[(TESTDATA$X<415 & TESTDATA$X>365 & TESTDATA$Y>365 & TESTDATA$Y<440) | (TESTDATA$X<425 & TESTDATA$X>330 & TESTDATA$Y>220 & TESTDATA$Y<330)]<-"Collaborative Work"
...
timeinfo<-summarise(times,
                    n=n(),
                    n_kitchen=sum(location == "Kitchen"]))
...
#interactions<-read.csv(file='BH_interactions.csv')
...
total_2$location[(total_2$x_avg<550 & total_2$x_avg>450 & total_2$y_avg>0 & total_2$y_avg<495) | (total_2$x_avg<370 & total_2$x_avg>310 & total_2$y_avg>0 & total_2$y_avg<70) | (total_2$x_avg<450 & total_2$x_avg>370 & total_2$y_avg>10 & total_2$y_avg<115)]<-"Desks"
total_2$location[total_2$x_avg<200 & total_2$x_avg>0 & total_2$y_avg>150 & total_2$y_avg<310]<-"Kitchen"
total_2$location[(total_2$x_avg<415 & total_2$x_avg>365 & total_2$y_avg>365 & total_2$y_avg<440) | (total_2$x_avg<425 & total_2$x_avg>330 & total_2$y_avg>220 & total_2$y_avg<330)]<-"Collaborative Work"
```
...
ggplot(data = subset(total_2,duration!=0),aes(x= -y_avg,y=-x_avg,color=location))+
  #scale_color_gradientn(colours = rainbow(10))+
  scale_x_continuous(limits = c(-520,110),breaks = seq(-520,110,10))+
  ...
  theme(legend.position = "none")+
  ggtitle("Plot of interactions based on location") +
  theme(plot.title = element_text(hjust = 0.5))
```
