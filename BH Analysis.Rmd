...
print(sprintf("The average daily journey distance for this particular day is  %4.0f m per employee\n", Average_Daily_Journey_Distance))
```
...
```{r}
sum(total_2$duration,total_2$location =="Kitchen" )
IE<-data.frame("Location" = c("Kitchen","Corridor","Meeting Room","Collaborative Work","Quiet Work","Desks"), "Number_of_Interactions" = c(sum(total_2$location == "Kitchen"),sum(total_2$location == "Corridor"),sum(total_2$location == "Meeting Room"),sum(total_2$location == "Collaborative Work"),sum(total_2$location == "Quiet Work"),sum(total_2$location == "Desks")), "Weighting_Factor" = c(ki,co,mr,cw,qw,de))
IE$Interaction_Impact<-IE$Number_of_Interactions*IE$Weighting_Factor
Interaction_Impact<-sum(IE$Interaction_Impact)
pdf("II.pdf", height=11, width=8.5)
grid.table(IE)
dev.off()
```
...
*Creating KPI Table:*
```{r}
KPIs<-data.frame("KPI" = c("Average Daily Journey Time (minutes)", "Average Daily Journey Distance (metres)", "Interaction Impact (no units)"),"Value"=c(Average_Daily_Journey_Time,Average_Daily_Journey_Distance,Interaction_Impact))
KPIs <- KPIs %>% 
